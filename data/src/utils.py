...
def get_data_directory_using_filters(dataset_tag, dataset_setting_json_path, down_sampled_ratio=None, up_sampled_ratio=None):
    """
    ...
    """
    assert down_sampled_ratio is None or up_sampled_ratio is None, "only one of down_sampled_ratio or up_sampled_ratio can be specified"

    main_path = f"cached/hvo_seq_data/{dataset_setting_json_path.split('/')[-1].replace('.', '__')}/{dataset_tag}/"
    last_directory = ""
    ...
    if down_sampled_ratio is None and up_sampled_ratio is None:
        return main_path + last_directory[:-1]  # remove last underline
    elif down_sampled_ratio is not None:
        return main_path + last_directory[:-1] + f"/_downsampled_{down_sampled_ratio}"
    elif up_sampled_ratio is not None:
        return main_path + last_directory[:-1] + f"/_upsampled_to_ensure_genre_balance"
