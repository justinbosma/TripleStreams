...
class HVO_Sequence(object):
    ...
    def find_index_for_pitch(self, pitch):
        ...
        for i, (k, v) in enumerate(self.drum_mapping.items()):
            if pitch in v:
                return i
        # print(f"[hvo_seq.py] Can't find pitch {pitch} in drum_mapping")
        return None
    ...
    def get_number_of_active_steps(self):
        hits = self.hits
        if self.is_ready_for_use() is False:
            return np.NaN
        return np.clip(np.count_nonzero(hits, axis=1), 0, 1).sum()

    def get_total_number_of_hits(self):
        if self.is_ready_for_use() is False:
            return np.NaN
        return np.count_nonzero(self.hits)
    ...
    def get_complexity_surprisal(self, lambda_=3.56, C=2.492082, D=-6.926637, kick_voice_idx=None, snare_voice_idx=None, cymbal_voice_idx=None):
        """
        Complexity and surprisal according to Senn:

        Senn O. A predictive coding approach to modelling the perceived complexity of popular music drum patterns. Heliyon. 2023 Apr 1;9(4).

        If indices are not provided:
            kick_voice_idx = [0]
            snare_voice_idx = [1]
            cymbal_voice_idx = [2, 3, 7, 8]

        :param kick_key (int or list):    key of kick voice in hvo_sequence (or list of keys)

        :return:
            complexity:             complexity value
            surprisal function:     an array of shape [65, 3] that can be used to calculate surprisal
        """
        ...
        # check 4-4 meter
        if self.grid_maker.beat_division_factors != [4] or self.time_signatures[0].denominator != 4:
            return np.NaN, np.NaN
        ...
        if isinstance(kick_voice_idx, int):
            ki = [kick_voice_idx]
        elif isinstance(kick_voice_idx, list):
            ki = kick_voice_idx
        if isinstance(snare_voice_idx, int):
            si = [snare_voice_idx]
        elif isinstance(snare_voice_idx, list):
            si = snare_voice_idx
        if isinstance(cymbal_voice_idx, int):
            ci = [cymbal_voice_idx]
        ...
        # flatten the voice dimensions
        kick_hits = kick_hits.sum(axis=-1)
        snare_hits = snare_hits.sum(axis=-1)
        cymbal_hits = cymbal_hits.sum(axis=-1)

        # trim to zero and one
        kick_hits = np.clip(kick_hits, 0, 1)
        snare_hits = np.clip(snare_hits, 0, 1)
        cymbal_hits = np.clip(cymbal_hits, 0, 1)

        Si = np.column_stack((kick_hits, snare_hits, cymbal_hits))

        # adapt Si length to 65
        if Si.shape[0] < 65:
            l_ = Si.shape[0]
            n_bars = l_ // 16 + 1
            # append zeros to ensure length is multiple of 16
            if l_ % 16 != 0:
                Si = np.append(Si, np.zeros((16 - l_ % 16, 3)), axis=0)
            # repeat Si n_bars times
            Si = np.tile(Si, (int(4 / n_bars) + 4, 1))
            Si = Si[:65, :]
            # extend with Si[0] to make it 65x3
        else:
            Si = Si[:65, :]
        ...
        # M0
        M0 = [
           [ 0.914165, 0.038968, 0.607162],
            [0.032583, 0.101896, 0.129147],
            [0.320498, 0.057464, 0.641588],
            [0.191943, 0.039692, 0.131517],
            [0.184242, 0.887441, 0.719194],
            [0.062796, 0.084123, 0.134479],
            [0.245853, 0.074052, 0.663507],
            [0.161730, 0.186611, 0.156398],
            [0.640995, 0.081754, 0.717417],
            [0.064573, 0.208531, 0.143365],
            [0.453791, 0.083531, 0.626185],
            [0.205569, 0.055687, 0.140995],
            [0.191351, 0.847156, 0.683057],
            [0.132701, 0.084716, 0.125592],
            [0.237559, 0.145142, 0.622630],
            [0.108412, 0.186611, 0.142773]
        ] * 4
        M0.append([0.914165, 0.038968, 0.607162])
        M0 = np.array(M0)
