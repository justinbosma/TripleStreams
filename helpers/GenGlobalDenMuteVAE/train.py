...
from helpers import genGlobalDenMuteVae_train_utils, genGlobalDenMuteVae_test_utils
...
parser.add_argument("--n_global_density_bins", type=int, help="Number of density bins (tokens)", default=10)
...
if args.config is not None:
    ...
    hparams = dict(
        ...
        n_global_density_bins=args.n_global_density_bins,
...
if __name__ == "__main__":
    ...
    training_dataset = Groove2Drum2BarDataset(
        ...
        num_voice_density_bins=config['n_global_density_bins']
    )
    ...
    # ------------------------------------------------------------------------------------------------------------
    genre_vae_cpu = GenreGlobalDensityWithVoiceMutesVAE(config)

    genre_vae = genre_vae_cpu.to(config.device)
    wandb.watch(genre_vae, log="all", log_freq=1)

    # Instantiate the loss Criterion and Optimizer
    # ------------------------------------------------------------------------------------------------------------

    #hit_loss_fn = torch.nn.BCEWithLogitsLoss(reduction='mean')
    hit_loss_fn = genGlobalDenMuteVae_train_utils.FocalLoss(alpha=config.alpha, gamma=config.gamma, reduction='mean')
    velocity_loss_fn = torch.nn.BCEWithLogitsLoss(reduction='mean')
    offset_loss_fn = torch.nn.HuberLoss(reduction='mean')

    if config.optimizer == 'adam':
        optimizer = torch.optim.Adam(genre_vae.parameters(), lr=config.lr)
    else:
        optimizer = torch.optim.SGD(genre_vae.parameters(), lr=config.lr)
    ...
    beta_np_cyc = genGlobalDenMuteVae_train_utils.generate_beta_curve(
    ...
    def batch_data_extractor(data_, device=config.device):
        ...
        global_density_bins = data_[6].to(device) if data_[6].device.type != device else data_[6]
        ...
        return inputs_, genre_tags, outputs_, global_density_bins, kick_is_muted, snare_is_muted, hat_is_muted, tom_is_muted, cymbal_is_muted

    def predict_using_batch_data(batch_data, model_, device=config.device):
        flat_hvo_groove, genre_tags, outputs_, global_density_bins, kick_is_muted, snare_is_muted, hat_is_muted, tom_is_muted, cymbal_is_muted = batch_data_extractor(batch_data, device)
        with torch.no_grad():
    ...
    for epoch in range(config.epochs):
        ...
        train_log_metrics, step_ = genGlobalDenMuteVae_train_utils.train_loop(
        ...
        # ---------------------------------------------------------------------------------------------------
        genre_vae.eval()       # DON'T FORGET TO SET THE MODEL TO EVAL MODE (check torch no grad)

        logger.info("***************************Testing...")

        test_log_metrics = genGlobalDenMuteVae_train_utils.test_loop(
        ...
        if args.piano_roll_samples:
            if epoch % args.piano_roll_frequency == 0:
                ...
                media = genGlobalDenMuteVae_test_utils.get_pianoroll_for_wandb(
                    model=genre_vae,
                ...
                media = genGlobalDenMuteVae_test_utils.generate_umap_for_vae_model_wandb(
                    model=genre_vae,
                    predict_using_batch_data=predict_using_batch_data,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='test',
        ...
        if args.calculate_hit_scores_on_train:
            if epoch % args.hit_score_frequency == 0:
                ...
                train_set_hit_scores = genGlobalDenMuteVae_test_utils.get_hit_scores_for_vae_model(
                    model=genre_vae,
                    predict_using_batch_data=predict_using_batch_data,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='train',
        ...
        if args.calculate_hit_scores_on_test:
            if epoch % args.hit_score_frequency == 0:
                ...
                test_set_hit_scores = genGlobalDenMuteVae_test_utils.get_hit_scores_for_vae_model(
                    model=genre_vae,
                    predict_using_batch_data=predict_using_batch_data,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
