...
def calculate_offset_loss(offset_logits, offset_targets, offset_loss_function):
    if isinstance(offset_loss_function, torch.nn.BCEWithLogitsLoss):
        ...
        loss_o = offset_loss_function(offset_logits, offset_targets+0.5)    # divide by 6 to scale the loss to match the other losses
...
def batch_loop(dataloader_, model, batch_data_extractor, hit_loss_fn, velocity_loss_fn,  offset_loss_fn,
               device, optimizer=None, starting_step=None, kl_beta=1.0, scale_h_loss=1.0, scale_v_loss=1.0, scale_o_loss=1.0):
    ...
    **Can be used for both training and testing. In testing however, backpropagation will not be performed**
    ...
    """
    assert isinstance(model, GenreWithVoiceMutesMultiTaskVAE), f"model must be an instance of GenreDensityTempoVAE. Got {type(model)}"
    ...
    for batch_count, (data_) in (pbar := tqdm.tqdm(enumerate(dataloader_), total=total_batches)):
        ...
        batch_loss_h = calculate_hit_loss(
            hit_logits=h_logits, hit_targets=h_targets, hit_loss_function=hit_loss_fn) * scale_h_loss

        batch_loss_v = calculate_velocity_loss(
            vel_logits=v_logits, vel_targets=v_targets, vel_loss_function=velocity_loss_fn) * scale_v_loss

        batch_loss_o = calculate_offset_loss(
            offset_logits=o_logits, offset_targets=o_targets, offset_loss_function=offset_loss_fn) * scale_o_loss
...
def train_loop(train_dataloader, model, batch_data_extractor,
               ...
               device, starting_step, kl_beta, scale_h_loss, scale_v_loss, scale_o_loss):
    """
...
def test_loop(test_dataloader, model, batch_data_extractor,
              ...
              device, kl_beta, scale_h_loss, scale_v_loss, scale_o_loss):
    """
