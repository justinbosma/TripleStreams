...
import torch
from model import GenreVAE
from helpers import genreVae_train_utils, genreVae_test_utils
...
if __name__ == "__main__":
    ...
    hit_loss_fn = genreVae_train_utils.FocalLoss(alpha=config.alpha, gamma=config.gamma, reduction='mean')
    ...
    beta_np_cyc = genreVae_train_utils.generate_beta_curve(
    ...
    for epoch in range(config.epochs):
        ...
        train_log_metrics, step_ = genreVae_train_utils.train_loop(
        ...
        test_log_metrics = genreVae_train_utils.test_loop(
        ...
        if args.piano_roll_samples:
            if epoch % args.piano_roll_frequency == 0:
                ...
                media = genreVae_test_utils.get_logging_media_for_vae_model_wandb(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='test',
                ...
                media = genreVae_test_utils.generate_umap_for_vae_model_wandb(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='test',
        ...
        if args.calculate_hit_scores_on_train:
            if epoch % args.hit_score_frequency == 0:
                ...
                train_set_hit_scores = genreVae_test_utils.get_hit_scores_for_vae_model(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='train',
        ...
        if args.calculate_hit_scores_on_test:
            if epoch % args.hit_score_frequency == 0:
                ...
                test_set_hit_scores = genreVae_test_utils.get_hit_scores_for_vae_model(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
