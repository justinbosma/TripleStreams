...
def generate_umap_for_vae_model_wandb(
        gen_vae, device, dataset_setting_json_path, subset_name,
    ...
    assert isinstance(gen_vae, GenreDensityTempoVAE)
    ...
    for batch_ix, batch_in in enumerate(dataloader):
        _, z = gen_vae.predict(flat_hvo_groove=batch_in[0].to(device), genre_tags=batch_in[4].to(device),
                               tempo_bins=batch_in[7].to(device),
                               kick_density_bins=batch_in[8].to(device),
                               snare_density_bins=batch_in[9].to(device),
                               hats_density_bins=batch_in[10].to(device),
                               toms_density_bins=batch_in[11].to(device),
                               cymbals_density_bins=batch_in[12].to(device))
...
def get_logging_media_for_vae_model_wandb(
    ...
    :param gen_vae: The model to be evaluated
    :param device: The device to be used for evaluation
    :param dataset_setting_json_path: The path to the dataset setting json file
    :param subset_name: The name of the subset to be evaluated
    ...
    genre_tags = evaluator.genre_tags
    genre_tags = torch.tensor(genre_tags, dtype=torch.long).to(device)
    tempo_bins = evaluator.tempo_bins
    tempo_bins = torch.tensor(tempo_bins, dtype=torch.long).to(device)
    kick_density_bins = evaluator.kick_density_bins
    kick_density_bins = torch.tensor(kick_density_bins, dtype=torch.long).to(device)
    snare_density_bins = evaluator.snare_density_bins
    snare_density_bins = torch.tensor(snare_density_bins, dtype=torch.long).to(device)
    hat_density_bins = evaluator.hats_density_bins
    hat_density_bins = torch.tensor(hat_density_bins, dtype=torch.long).to(device)
    tom_density_bins = evaluator.toms_density_bins
    tom_density_bins = torch.tensor(tom_density_bins, dtype=torch.long).to(device)
    cymbal_density_bins = evaluator.cymbals_density_bins
    cymbal_density_bins = torch.tensor(cymbal_density_bins, dtype=torch.long).to(device)

    hvos_array, latent_z = gen_vae.predict(flat_hvo_groove=in_groove,
                                           genre_tags=genre_tags,
                                           tempo_bins=tempo_bins,
                                           kick_density_bins=kick_density_bins,
                                           snare_density_bins=snare_density_bins,
                                           hats_density_bins=hat_density_bins,
                                           toms_density_bins=tom_density_bins,
                                           cymbals_density_bins=cymbal_density_bins)
...
def get_hit_scores_for_vae_model(gen_vae, device, dataset_setting_json_path, subset_name,
                                 down_sampled_ratio, collapse_tapped_sequence,
                                 cached_folder="cached/Evaluators/templates/",
    ...
    in_groove = torch.tensor(
        np.array([hvo_seq.flatten_voices(reduce_dim=collapse_tapped_sequence)
                  for hvo_seq in hvo_seqs]), dtype=torch.float32).to(
        device)
    genre_tags = torch.tensor(evaluator.genre_tags, dtype=torch.long).to(device)
    tempo_bins = torch.tensor(evaluator.tempo_bins, dtype=torch.long).to(device)
    kick_density_bins = torch.tensor(evaluator.kick_density_bins, dtype=torch.long).to(device)
    snare_density_bins = torch.tensor(evaluator.snare_density_bins, dtype=torch.long).to(device)
    hat_density_bins = torch.tensor(evaluator.hats_density_bins, dtype=torch.long).to(device)
    tom_density_bins = torch.tensor(evaluator.toms_density_bins, dtype=torch.long).to(device)
    cymbal_density_bins = torch.tensor(evaluator.cymbals_density_bins, dtype=torch.long).to(device)

    # load tags and tempos
    ...
    gen_vae.eval()
    split_size = 64
    splitted_in_groove = torch.split(in_groove, split_size)
    splitted_genre_tags = torch.split(genre_tags, split_size)
    splitted_tempo_bins = torch.split(tempo_bins, split_size)
    splitted_kick_density_bins = torch.split(kick_density_bins, split_size)
    splitted_snare_density_bins = torch.split(snare_density_bins, split_size)
    splitted_hat_density_bins = torch.split(hat_density_bins, split_size)
    splitted_tom_density_bins = torch.split(tom_density_bins, split_size)
    splitted_cymbal_density_bins = torch.split(cymbal_density_bins, split_size)

    with torch.no_grad():
        for batch_ix in range(len(splitted_in_groove)):
            in_ = splitted_in_groove[batch_ix].to(device)
            gt_ = splitted_genre_tags[batch_ix].to(device)
            hvos_array, latent_z = gen_vae.predict(flat_hvo_groove=in_, genre_tags=gt_,
                                                    tempo_bins=splitted_tempo_bins[batch_ix].to(device),
                                                    kick_density_bins=splitted_kick_density_bins[batch_ix].to(device),
                                                    snare_density_bins=splitted_snare_density_bins[batch_ix].to(device),
                                                    hats_density_bins=splitted_hat_density_bins[batch_ix].to(device),
                                                    toms_density_bins=splitted_tom_density_bins[batch_ix].to(device),
                                                    cymbals_density_bins=splitted_cymbal_density_bins[batch_ix].to(device))
