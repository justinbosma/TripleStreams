...
def calculate_hit_loss(hit_logits, hit_targets, hit_loss_function):
    ...
    loss_h = hit_loss_function(hit_logits, hit_targets) * 10           # batch, time steps, voices (10 is a scaling factor to match the other losses)
...
def calculate_offset_loss(offset_logits, offset_targets, offset_loss_function):
    if isinstance(offset_loss_function, torch.nn.BCEWithLogitsLoss):
        ...
        loss_o = offset_loss_function(offset_activated, offset_targets) * 10 # batch, time steps, voices (10 is a scaling factor to match the other losses)
    else:
...
def batch_loop(dataloader_, model, hit_loss_fn, velocity_loss_fn,  offset_loss_fn,
    ...
    **Can be used for both training and testing. In testing however, backpropagation will not be performed**
    ...
    :param model:  (GenreTempoDensityVAE)  the model
    ...
    """
    assert isinstance(model, GenreDensityTempoVAE), f"model must be an instance of GenreDensityTempoVAE. Got {type(model)}"
    ...
    for batch_count, (data_) in (pbar := tqdm.tqdm(enumerate(dataloader_), total=total_batches)):
        ...
        outputs_ = data_[5]
        tempo_bins = data_[7].to(device) if data_[7].device.type != device else data_[7]
        kick_density_bin = data_[8].to(device) if data_[8].device.type != device else data_[8]
        snare_density_bin = data_[9].to(device) if data_[9].device.type != device else data_[9]
        hats_density_bin = data_[10].to(device) if data_[10].device.type != device else data_[10]
        toms_density_bin = data_[11].to(device) if data_[11].device.type != device else data_[11]
        cymbal_density_bin = data_[12].to(device) if data_[12].device.type != device else data_[12]

        # Move data to GPU if available
        # ---------------------------------------------------------------------------------------
        inputs = inputs_.to(device) if inputs_.device.type!= device else inputs_
        genre_tags = genre_tags.to(device) if genre_tags.device.type!= device else genre_tags
        ...
        h_logits, v_logits, o_logits, mu, log_var, latent_z = model.forward(
            ...
            tempo_bins=tempo_bins, kick_density_bins=kick_density_bin,
            snare_density_bins=snare_density_bin,
            hats_density_bins=hats_density_bin,
            toms_density_bins=toms_density_bin,
            cymbals_density_bins=cymbal_density_bin)
