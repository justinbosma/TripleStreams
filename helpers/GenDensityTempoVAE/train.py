...
from helpers import genDensityTempoVae_train_utils, genDensityTempoVae_test_utils
...
parser.add_argument("--n_density_bins", type=int, help="Number of density bins (tokens)", default=3)
parser.add_argument("--n_tempo_bins", type=int, help="Number of tempo bins (tokens) - only 6 supported atm", default=6)
...
parser.add_argument("--dataset_json_fname", type=str,
                    ...
                    default="Balanced_5000_per_genre_performed_4_4.json")
...
if args.config is not None:
    ...
    hparams = dict(
        ...
        n_tempo_bins=args.n_tempo_bins,
...
if __name__ == "__main__":
    ...
    training_dataset = Groove2Drum2BarDataset(
        ...
        num_voice_density_bins=config['n_density_bins'],
        num_tempo_bins=config['n_tempo_bins']
    )
    ...
    # ------------------------------------------------------------------------------------------------------------
    genre_vae_cpu = GenreDensityTempoVAE(config)
    ...
    hit_loss_fn = genDensityTempoVae_train_utils.FocalLoss(alpha=config.alpha, gamma=config.gamma, reduction='mean')
    ...
    beta_np_cyc = genDensityTempoVae_train_utils.generate_beta_curve(
    ...
    for epoch in range(config.epochs):
        ...
        train_log_metrics, step_ = genDensityTempoVae_train_utils.train_loop(
        ...
        test_log_metrics = genDensityTempoVae_train_utils.test_loop(
        ...
        if args.piano_roll_samples:
            if epoch % args.piano_roll_frequency == 0:
                ...
                media = genDensityTempoVae_test_utils.get_logging_media_for_vae_model_wandb(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='test',
                    down_sampled_ratio=0.07,
                    collapse_tapped_sequence=collapse_tapped_sequence,
                ...
                media = genDensityTempoVae_test_utils.generate_umap_for_vae_model_wandb(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='test',
        ...
        if args.calculate_hit_scores_on_train:
            if epoch % args.hit_score_frequency == 0:
                ...
                train_set_hit_scores = genDensityTempoVae_test_utils.get_hit_scores_for_vae_model(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='train',
        ...
        if args.calculate_hit_scores_on_test:
            if epoch % args.hit_score_frequency == 0:
                ...
                test_set_hit_scores = genDensityTempoVae_test_utils.get_hit_scores_for_vae_model(
                    gen_vae=genre_vae,
                    device=config.device,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
