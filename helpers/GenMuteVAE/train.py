...
from helpers import genMuteVae_train_utils, genMuteVae_test_utils
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------------------------------------------------
    genre_vae_cpu = GenreWithVoiceMutesVAE(config)
    ...
    # hit_loss_fn = genMuteVae_train_utils.FocalLoss(alpha=config.alpha, gamma=config.gamma, reduction='mean')
    hit_loss_fn = genMuteVae_train_utils.DiceLoss(reduction='mean')
    ...
    beta_np_cyc = genMuteVae_train_utils.generate_beta_curve(
    ...
    for epoch in range(config.epochs):
        ...
        train_log_metrics, step_ = genMuteVae_train_utils.train_loop(
        ...
        test_log_metrics = genMuteVae_train_utils.test_loop(
        ...
        if args.piano_roll_samples:
            if epoch % args.piano_roll_frequency == 0:
                ...
                media, previous_evaluator_for_piano_rolls = genMuteVae_test_utils.get_pianoroll_for_wandb(
                    model=genre_vae,
                ...
                media, previous_loaded_dataset_for_umap_test = genMuteVae_test_utils.generate_umap_for_vae_model_wandb(
                    model=genre_vae,
                    predict_using_batch_data=predict_using_batch_data,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='test',
                ...
                media, previous_loaded_dataset_for_umap_train = genMuteVae_test_utils.generate_umap_for_vae_model_wandb(
                    model=genre_vae,
                    predict_using_batch_data=predict_using_batch_data,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='train',
        ...
        if args.calculate_hit_scores_on_train:
            if epoch % args.hit_score_frequency == 0:
                ...
                train_set_hit_scores, previous_evaluator_for_hit_scores_train = genMuteVae_test_utils.get_hit_scores_for_vae_model(
                    model=genre_vae,
                    predict_using_batch_data=predict_using_batch_data,
                    dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
                    subset_name='train',
        ...
        if args.calculate_hit_scores_on_test:
            ...
            test_set_hit_scores, previous_evaluator_for_hit_scores_test = genMuteVae_test_utils.get_hit_scores_for_vae_model(
                model=genre_vae,
                predict_using_batch_data=predict_using_batch_data,
                dataset_setting_json_path=f"{config.dataset_json_dir}/{config.dataset_json_fname}",
