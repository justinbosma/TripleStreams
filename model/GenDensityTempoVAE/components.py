...
class DecoderInput(torch.nn.Module):
    ...
    def __init__ (self, max_len, latent_dim, d_model, n_genres, n_tempo_bins, n_density_bins):
        super(DecoderInput, self).__init__()
        self.max_len = max_len
        self.d_model = d_model
        self.n_genres = n_genres
        self.n_tempo_bins = n_tempo_bins
        self.n_density_bins = n_density_bins + 1 # +1 for the zero density bin

        self.genre_embedding = torch.nn.Embedding(num_embeddings=self.n_genres, embedding_dim=latent_dim)
        self.tempo_embedding = torch.nn.Embedding(num_embeddings=self.n_tempo_bins, embedding_dim=latent_dim)
        self.kick_density_embedding = torch.nn.Embedding(num_embeddings=self.n_density_bins, embedding_dim=latent_dim)
        self.snare_density_embedding = torch.nn.Embedding(num_embeddings=self.n_density_bins, embedding_dim=latent_dim)
        self.hats_density_embedding = torch.nn.Embedding(num_embeddings=self.n_density_bins, embedding_dim=latent_dim)
        self.toms_density_embedding = torch.nn.Embedding(num_embeddings=self.n_density_bins, embedding_dim=latent_dim)
        self.cymbals_density_embedding = torch.nn.Embedding(num_embeddings=self.n_density_bins, embedding_dim=latent_dim)
    ...
    def init_weights(self, initrange=0.1):
        ...
        self.tempo_embedding.weight.data.uniform_(-initrange, initrange)
        self.kick_density_embedding.weight.data.uniform_(-initrange, initrange)
        self.snare_density_embedding.weight.data.uniform_(-initrange, initrange)
        self.hats_density_embedding.weight.data.uniform_(-initrange, initrange)
        self.toms_density_embedding.weight.data.uniform_(-initrange, initrange)
        self.cymbals_density_embedding.weight.data.uniform_(-initrange, initrange)
    ...
    def forward(self,
        ...
        :param tempo_bins:  shape (batch) or (batch, 1)
        :param kick_density_bins:  shape (batch) or (batch, 1)
        :param snare_density_bins:  shape (batch) or (batch, 1)
        :param hats_density_bins:  shape (batch) or (batch, 1)
        :param toms_density_bins:  shape (batch) or (batch, 1)
        :param cymbals_density_bins:  shape (batch) or (batch, 1)
        :return:
        ...
        if len(tempo_bins.shape) == 2:
            tempo_bins = tempo_bins.squeeze(-1)
        if len(kick_density_bins.shape) == 2:
            kick_density_bins = kick_density_bins.squeeze(-1)
        if len(snare_density_bins.shape) == 2:
            snare_density_bins = snare_density_bins.squeeze(-1)
        if len(hats_density_bins.shape) == 2:
            hats_density_bins = hats_density_bins.squeeze(-1)
        if len(toms_density_bins.shape) == 2:
            toms_density_bins = toms_density_bins.squeeze(-1)
        if len(cymbals_density_bins.shape) == 2:
            cymbals_density_bins = cymbals_density_bins.squeeze(-1)

        genres_embedded = self.genre_embedding(genre_tags)
        tempo_embedded = self.tempo_embedding(tempo_bins)

        density_embedded = self.kick_density_embedding(kick_density_bins) + \
                           self.snare_density_embedding(snare_density_bins) + \
                           self.hats_density_embedding(hats_density_bins) + \
                           self.toms_density_embedding(toms_density_bins) + \
                           self.cymbals_density_embedding(cymbals_density_bins)

        latent_z = latent_z + genres_embedded + tempo_embedded + density_embedded
