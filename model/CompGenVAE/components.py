...
class GenreLayer(torch.nn.Module):
    """
    Converts the output of the transformer into genre logits (i.e. Values **PRIOR** to activation without softmax)
    """

    def __init__ (self, max_len, d_model, n_genres):
        super(GenreLayer, self).__init__()
        dim_ = max_len * d_model
        self.fc1 = torch.nn.Linear(dim_, dim_//2)
        self.relu1 = torch.nn.ReLU()
        self.fc2 = torch.nn.Linear(dim_//2, dim_//4)
        self.relu2 = torch.nn.ReLU()
        self.fc3 = torch.nn.Linear(dim_//4, dim_//8)
        self.relu3 = torch.nn.ReLU()
        self.fc = torch.nn.Linear(dim_//8, n_genres)
    ...
    @torch.jit.export
    def forward(self, src: torch.Tensor):
        t = torch.flatten(src, start_dim=1)
        l1 = self.relu1(self.fc1(t))
...
class ComplexityLayer(torch.nn.Module):
    """
    Predicts the complexity of the input sequence using only the flattened hits of the GROOVE!

    Note: Logit is regression output, not classification output (although training is done with BCE loss)
    """

    def __init__ (self, max_len, d_model):
        super(ComplexityLayer, self).__init__()
        dim_ = max_len * d_model
        ...
        self.fc3 = torch.nn.Linear(dim_//4, dim_//8)
        self.relu3 = torch.nn.ReLU()
        self.fc4 = torch.nn.Linear(dim_ // 8, 1)
...
class CompGenDecoderInput(torch.nn.Module):
    """ reshape the input tensor to fix dimensions with decoder

    [latent, complexity(after sigmoid), genre(after softmax)]

    """
    def __init__ (self, max_len, latent_dim, d_model, n_genres):
        super(CompGenDecoderInput, self).__init__()

        self.max_len = max_len
        self.d_model = d_model
        self.n_genres = n_genres
        self.fc = torch.nn.Linear(int(latent_dim + 1 + n_genres), int(max_len * d_model))
    ...
    def forward(self,
        ...
        :param complexity_logits:  shape (batch, 1) or (batch)
        :param genre_logits:  shape (batch, n_genres)
        :return:
        """

        if complexity_logits.shape[-1] != 1:
            print("complexity_logits.shape[-1] != 1, maybe mixed up with genre?")

        if len(complexity_logits.shape) == 1:
            complexity_logits = complexity_logits[:, None]

        genre_probs = self.softmax(genre_logits.detach())
        complexity_vals = torch.sigmoid(complexity_logits.detach())
        i_ = torch.cat((latent_z, complexity_vals, genre_probs), dim=-1)
        uptensor = self.fc.forward(i_)
        result = uptensor.view(-1, self.max_len, self.d_model)

        return result

    @torch.jit.export
    def forward_without_activation(self, latent_z: torch.Tensor, complexity: torch.Tensor, genre: torch.Tensor):
        """
        does not apply the activation functions but  reshapes the input tensor to fix dimensions with decoder
        :param latent_z: shape (batch, latent_dim)
        :param complexity_logits:  shape (batch, 1) or (batch)
        :param genre_logits:  shape (batch, n_genres)
        :return:
        ...
        if complexity.shape[-1] != 1:
            print("complexity.shape[-1] != 1, maybe mixed up with genre?")

        if len(complexity.shape) == 1:
            complexity = complexity[:, None]

        input = torch.cat((latent_z,  complexity, genre), dim=-1)
        uptensor = self.fc.forward(input)
        result = uptensor.view(-1, self.max_len, self.d_model)
