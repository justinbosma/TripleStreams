...
from model import GenreWithVoiceMutesMultiTaskComponents


class GenreWithVoiceMutesMultiTaskVAE(torch.nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        super(GenreWithVoiceMutesMultiTaskVAE, self).__init__()
        ...
        # ---------------------------------------------------
        self.InputLayerEncoder = GenreWithVoiceMutesMultiTaskComponents.InputGrooveLayer(
        ...
        self.Encoder = GenreWithVoiceMutesMultiTaskComponents.Encoder(
            d_model=self.config['d_model_enc'],
        ...
        self.latentLayer = GenreWithVoiceMutesMultiTaskComponents.LatentLayer(
            max_len=self.config['max_len_enc'],
            d_model=self.config['d_model_enc'],
            latent_dim=self.config['latent_dim']
        )

        self.HitsDecoderInput = GenreWithVoiceMutesMultiTaskComponents.DecoderInput(
        ...
        self.HitsDecoder = GenreWithVoiceMutesMultiTaskComponents.Encoder(
            d_model=self.config['d_model_dec'],
        ...
        self.HitsOutputLayer = GenreWithVoiceMutesMultiTaskComponents.SingleFeatureOutputLayer(
            embedding_size=self.config['embedding_size_tgt'] // 3,
            d_model=self.config['d_model_dec'],
        )
        
        self.velocityDecoderInput = GenreWithVoiceMutesMultiTaskComponents.DecoderInput(
            max_len=self.config['max_len_dec'],
            latent_dim=self.config['latent_dim'],
            d_model=self.config['d_model_dec'],
            n_genres=self.config['n_genres']
        ...
        self.VelocityDecoder = GenreWithVoiceMutesMultiTaskComponents.Encoder(
            d_model=self.config['d_model_dec'],
            nhead=self.config['nhead_dec'],
            dim_feedforward=self.config['dim_feedforward_dec'],
            num_encoder_layers=self.config['num_decoder_layers'],
        ...
        self.VelocityOutputLayer = GenreWithVoiceMutesMultiTaskComponents.SingleFeatureOutputLayer(
            embedding_size=self.config['embedding_size_tgt'] // 3,
            d_model=self.config['d_model_dec'],
        )

        self.OffsetDecoderInput = GenreWithVoiceMutesMultiTaskComponents.DecoderInput(
            max_len=self.config['max_len_dec'],
            latent_dim=self.config['latent_dim'],
            d_model=self.config['d_model_dec'],
            n_genres=self.config['n_genres']
        ...
        self.OffsetDecoder = GenreWithVoiceMutesMultiTaskComponents.Encoder(
            d_model=self.config['d_model_dec'],
            nhead=self.config['nhead_dec'],
            dim_feedforward=self.config['dim_feedforward_dec'],
            num_encoder_layers=self.config['num_decoder_layers'],
        ...
        self.OffsetOutputLayer = GenreWithVoiceMutesMultiTaskComponents.SingleFeatureOutputLayer(
            embedding_size=self.config['embedding_size_tgt'] // 3,
            d_model=self.config['d_model_dec'],
        )
    ...
    @torch.jit.export
    def decode(self, latent_z: torch.Tensor, genre_tags: torch.Tensor,
...
if __name__ == "__main__":
    ...
    model = GenreWithVoiceMutesMultiTaskVAE(config)
