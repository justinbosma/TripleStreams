...
class OutputLayer(torch.nn.Module):
    ...
    def forward(self, decoder_out):
        ...
        hvo_logits = torch.reshape(hvo_logits,
                                   (decoder_out.shape[0], decoder_out.shape[1], 3, self.embedding_size // 3))
...
class DecoderInput(torch.nn.Module):
    ...
    def __init__(self, max_len, latent_dim, d_model, n_genres, n_global_density_bins):
        super(DecoderInput, self).__init__()
        self.max_len = max_len
        self.d_model = d_model
        self.n_genres = n_genres
        self.n_global_density_bins = n_global_density_bins + 1  # +1 for the zero density bin

        self.genre_embedding = torch.nn.Embedding(num_embeddings=self.n_genres, embedding_dim=latent_dim)
        self.global_density_embedding = torch.nn.Embedding(num_embeddings=self.n_global_density_bins,
    ...
    def init_weights(self, initrange=0.1):
        ...
        self.global_density_embedding.weight.data.uniform_(-initrange, initrange)
    ...
    def forward(self,
        ...
        :param global_density_bins:  shape (batch) or (batch, 1)
        ...
        if len(global_density_bins.shape) == 2:
            global_density_bins = global_density_bins.squeeze(-1)
        ...
        latent_z = (latent_z + self.genre_embedding(genre_tags) + self.global_density_embedding(global_density_bins) +
