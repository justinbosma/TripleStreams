...
from model import GenGlobalDenMuteComponents


class GenreGlobalDensityWithVoiceMutesVAE(torch.nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        super(GenreGlobalDensityWithVoiceMutesVAE, self).__init__()
        ...
        # ---------------------------------------------------
        self.InputLayerEncoder = GenGlobalDenMuteComponents.InputGrooveLayer(
        ...
        self.Encoder = GenGlobalDenMuteComponents.Encoder(
            d_model=self.config['d_model_enc'],
        ...
        self.latentLayer = GenGlobalDenMuteComponents.LatentLayer(
            max_len=self.config['max_len_enc'],
            d_model=self.config['d_model_enc'],
            latent_dim=self.config['latent_dim']
        )

        self.DecoderInput = GenGlobalDenMuteComponents.DecoderInput(
            max_len=self.config['max_len_dec'],
            latent_dim=self.config['latent_dim'],
            d_model=self.config['d_model_dec'],
            n_genres=self.config['n_genres'],
            n_global_density_bins=self.config['n_global_density_bins']
        )

        self.Decoder = GenGlobalDenMuteComponents.Encoder(
            d_model=self.config['d_model_dec'],
        ...
        self.OutputLayer = GenGlobalDenMuteComponents.OutputLayer(
    ...
    def decode(self, latent_z: torch.Tensor, genre_tags: torch.Tensor,
               global_density_bins: torch.Tensor,
        ...
        :param global_density_bins: [N, 10]
...
if __name__ == "__main__":
    ...
    model = GenreGlobalDensityWithVoiceMutesVAE(config)
    genre_tags = torch.tensor([[1]])
    global_density_bins = torch.tensor([3])
