...
from model import GenreComponents


class GenreVAE(torch.nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        # Layers
        # ---------------------------------------------------
        self.InputLayerEncoder = GenreComponents.InputGrooveLayer(
        ...
        self.Encoder = GenreComponents.Encoder(
            d_model=self.config['d_model_enc'],
        ...
        self.latentLayer = GenreComponents.LatentLayer(
            max_len=self.config['max_len_enc'],
            d_model=self.config['d_model_enc'],
            latent_dim=self.config['latent_dim']
        )

        self.DecoderInput = GenreComponents.GenreDecoderInput(
        ...
        self.Decoder = GenreComponents.Encoder(
            d_model=self.config['d_model_dec'],
        ...
        self.OutputLayer = GenreComponents.OutputLayer(
    ...
    @torch.jit.export
    def forward(self, flat_hvo_groove: torch.Tensor, genre_tags: torch.Tensor):
        ...
        **pre-activation** output sequence of shape (batch_size, seq_len, embedding_size_tgt)
        ...
        h_logits, v_logits, o_logits, hvo_logits = self.decode(latent_z=latent_z, genre_tags=genre_tags)
...
if __name__ == "__main__":
    ...
    model = GenreVAE(config)
    model.forward(torch.rand(1, 32, 3), torch.tensor([1]))
